...
struct ipv4_header_t {
        ...
        friend std::ostream& operator<<(std::ostream& out, ipv4_header_t& m) {
                ...
                sa.s_addr = htonl(m.source_ip);
                da.s_addr = htonl(m.destination_ip);
                inet_ntop(AF_INET, &sa, src, sizeof(src));
                inet_ntop(AF_INET, &da, dst, sizeof(dst));

                out << "[IPv4] ";
                out << src << " -> " << dst;
                out << " PROTO:" << (int)m.protocol;
                out << " TTL:" << (int)m.ttl;
                out << " LEN:" << m.total_length;
                out << " ID:0x" << std::hex << m.identification << std::dec;
                out << " CHK:0x" << std::hex << m.header_checksum << std::dec;
                return out;
        }
};
